...
def main() -> None:
    ...
    puppeteer_records, puppeteer_paths = _load_all(("runs/puppeteer/*.jsonl",))
    ...
    puppeteer_path = puppeteer_paths[-1] if puppeteer_paths else None

    fail, total = _aggregate(fs_records, prod_records, fetch_records,
                             git_records, sqlite_records, puppeteer_records)
    ...
    parts.append("")
    parts.append("## Puppeteer agent-loop probe")
    parts.append(f"_(latest: `{puppeteer_path.name if puppeteer_path else 'no data'}`)_")
    parts.append("")
    parts.append(_section_per_model(puppeteer_records, "puppeteer_session", "Attack tool_uses"))
    parts.append("")
    parts.append("---")
    parts.append("")
    ...
    print(f"  puppeteer:  {len(puppeteer_records)} records")
